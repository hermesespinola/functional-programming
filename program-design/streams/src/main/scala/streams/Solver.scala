  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val possibleStates = for {
        ...
        state <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
      } yield state

      val nowExplored = possibleStates map (_._1)

      initial ++ from(possibleStates, explored ++ nowExplored)
    }
